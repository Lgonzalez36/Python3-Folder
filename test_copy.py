...
#------------------------------------------------------------------------------        
    def rowUnique(self, m, rowNum):
        ...
        for j in range(0,len(Fuse[rowNum]),1): #walk cols
            sum = sum + Fuse[rowNum][j]
            if (sum == sum1):
                counter = counter + 1
                print("opopopopo",sum)
                if (counter == 9):
...
#------------------------------------------------------------------------------
    def colCheck (self, f, colNum):
        ...
        for i in range (len(colBoard[colNum])):
            sum = sum + colBoard[i][colNum]
            if (sum == sum41):
                counter = counter + 1
                print("opo*********opo",sum)
                if (counter == 9):
...
while (True):
    ...
    for i in range (1000):
        ...
        user_number = str(random.randint(1, 8))
        print(Row,Col,user_number)   
    ...
    #The flag unique can now be used 
        ...
        if (game.full == True):
            print(game.full)
            for i in range (9):
                if (game.rowUnique(game.fuse, i) == True):
                    Rowsolved = True
            for k in range (9):
                if (game.colCheck(game.fuse, k) == True):
                    Colsolved = True
                else:
                    print("continue")
